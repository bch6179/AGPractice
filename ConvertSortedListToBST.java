...
class ConvertSortedListToBST {
    ...
    /**
     * Bottom up approach, O(n) (Instead of top-down, O(nlogn))   //in order traversal, curve
     *
    ...
    public static TreeNode buildTree(int n) {
        ...
        node.val = cur.val;
        cur = cur.next;
        node.right = buildTree(n - n / 2 - 1); // why n - n/2 - 1? The # of nodes in right subtree is total nodes - nodes in left subtree - root
        return node;
    }
}