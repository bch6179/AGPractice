...
# otherwise, res+= j-i when seeing valid parenthese  bad
        ...
        for(int j=0;j<s.length();j++){
            if(s.charAt(j)=='(') stack.push(j);            
                ...
                if (stack.isEmpty()) left=j;
                    ...
                    else max=Math.max(max,j-stack.peek()); #Mistake (() one of my mistake , not considering this case; res += not correct for (()) 2+4, duojiu le liange
                }
            }
        }
        return max;
