    ...
    public static List<Integer> inorderTraversal(TreeNode root) {
        ...
        while (!s.isEmpty() || root != null) {
            ...
            if (root != null) { // current node is not null  
                //push all to the left of the current right node
                s.push(root);
                root = root.left;
            } else { // current node is null, pop next left edge in stack, check and go right
                     // left edge except leaf one will be visited twice
                root = s.pop();  // get the leaf
                result.add(root.val); // visit()
                // check prev and root.val and update prev
                root = root.right;
            }
        }
        return result;
